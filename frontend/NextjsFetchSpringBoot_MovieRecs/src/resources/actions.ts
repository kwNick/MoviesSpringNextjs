...
export async function AddMovie(prevState: State | undefined, formData: FormData) {
    ...
    try {
        await fetch(`http://${process.env.ROUTER_API_IP}:${process.env.REST_API_PORT}/newmovie`, {
            method: 'POST',
            headers: {
...
export async function UpdateMovie(id: string, prevState: PatchState | undefined, formData: FormData) {
    ...
    try {
        await fetch(`http://${process.env.ROUTER_API_IP}:${process.env.REST_API_PORT}/newmovie/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
