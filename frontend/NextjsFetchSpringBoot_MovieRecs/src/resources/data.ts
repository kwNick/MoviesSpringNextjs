...
export async function CountMoviesByTitle(col: string, query?: string, page?: number, size?: number) {
    try {
        ...
        page = page ?? 0;
        size = size ?? 5;
        const data = await fetch(`https://${process.env.SPRING_API_DOMAIN}/${col}/search/countByTitleIgnoreCaseLike?title=${query}&page=${page - 1}&size=${size}`);
        return data.json();
    } catch (error) {
...
export async function FindByTitleLike(col: string, query?: string, page?: number, size?: number) {
    try {
        ...
        page = page ?? 0;
        size = size ?? 5;
        const data = await fetch(`https://${process.env.SPRING_API_DOMAIN}/${col}/search/findByTitleIgnoreCaseLike?title=${query}&page=${page - 1}&size=${size}`);
        return data.json();
    } catch (error) {
