...
const SlidingMarquee = async () => {
    const size = 10
    const movies = await FindByTitleLike(undefined, undefined, size);
    return (
        <>
                ...
                <div className="flex">
                    <p className="px-5 animate-slidingMarquee group-active:[animation-play-state:paused]">
                        ...
                        {movies._embedded.movie.map((x: Movie, idx: number) => {
                            return (
                                <span key={idx} className="hover:text-sm hover:text-snow duration-300"> #{x.movieId}. {x.title} - rating: {x.averageRating} - {x.releaseYear} | </span>
                            );
                        })}
                    </p>
                    <p className="px-5 animate-slidingMarquee group-active:[animation-play-state:paused]">
